...
def run(config, shell):
  if config.NEXTALIGN_BUILD_CLI:
    shell(f"eval {config.GDB} {config.NEXTALIGN_CLI_EXE} {config.NEXTALIGN_CLI_ARGS} || true")
  ...
  if config.NEXTCLADE_BUILD_CLI:
    ...
    shell(f"eval {config.GDB} {config.NEXTCLADE_CLI_EXE} {config.NEXTCLADE_CLI_ARGS} || true")
