...
use crate::utils::range::{intersect_or_none, NucRefGlobalRange, Range};
use eyre::Report;
...
pub fn calculate_aa_alignment_ranges_in_place(
  ...
  translation.iter_cdses_mut().try_for_each(|(_, cds_tr)| {
    ...
    //     // let segment_global_aln_range = coord_map_global.ref_to_aln_range(&segment.range);
    //
    //     // Trim segment to the nuc alignment range
    //     intersect_or_none(&segment.range, alignment_range).map(|segment_range_global_aln| {
    //       let segment_start_pos_global_aln = coord_map_global.ref_to_aln_position(segment.start);
