...
class PocoConan(ConanFile):
    ...
    def source(self):
        tools.get("https://github.com/pocoproject/poco/archive/poco-1.11.0-release.tar.gz",
                  destination=self._source_subfolder, strip_root=True)

        if not self.options.enable_active_record:
            # Looks like it still tries to build ActiveRecord, even when disabled. Deleting the sources fixes that.
            shutil.rmtree(os.path.join(self._source_subfolder, "ActiveRecord"))
