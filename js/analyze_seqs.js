...
function isNodeInClade(cladeAlleles, mutations, reference) {
  const conditions = cladeAlleles.map((d) => {
    const state =
      mutations[d.pos] === undefined ? reference[d.pos - 1] : mutations[d.pos];
...
d3.select("#seqinput").on("keyup", function () {
  ...
  Object.keys(seqs).forEach((element) => {
    ...
    mutations = analyzeSeq(seqs[element]);
    const assignedClades = Object.keys(clades).filter((clade) =>
      isNodeInClade(clades[clade], mutations, rootSeq)
    );
    console.log("CLADES", assignedClades);
    document.getElementById(
      "results"
    ).innerHTML += `${element}: ${assignedClades}<br>`;
  });
});
