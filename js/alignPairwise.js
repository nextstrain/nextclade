function alignPairwise(query, ref) {
  ...
  if (bandWidth > 2 * seedLength) {
    ...
    for (let ni = 0; ni < nSeeds; ni++) {
      ...
      [tmpShift, tmpScore] = seedMatch(
        query.substring(qPos, qPos + seedLength)
      );
  ...
  for (ri = 0; ri < ref.length; ri++) {
    for (si = 2 * bandWidth; si >= 0; si--) {
      ...
      if (qPos >= 0 && qPos < query.length) {
        tmpMatch =
          ref[ri] === query[qPos] || query[qPos] === "N" ? match : misMatch;
        rGapOpen =
          si < 2 * bandWidth ? (paths[si + 1][ri + 1] === 2 ? 0 : gapOpen) : 0;
  ...
  const aln = [];
  const lastIndexByShift = scores.map((d, i) =>
    d3.min([rowLength - 1, query.length + indexToShift(i)])
  );
