...
auto parseCommandLine(int argc, char *argv[]) {// NOLINT(cppcoreguidelines-avoid-c-arrays)
    ...
    if (result.count("output-json") == 0 && result.count("output-csv") == 0 && result.count("output-tsv") == 0 &&
        ...
        std::cerr
                << "Error: at least one of output path arguments required: --output-json, --output-csv, --output-tsv, "
                   "--output-tree"
                << std::endl;
        std::cerr << options.help() << std::endl;
        std::exit(1);
...
Params validateParams(const CliOptions &options) {
    ...
    return {numThreads, inputFasta, inputRootSeq, inputTree, inputQcConfig, inputGeneMap, inputPcrPrimers,
            outputJson, outputCsv, outputTsv, outputTree};
}
