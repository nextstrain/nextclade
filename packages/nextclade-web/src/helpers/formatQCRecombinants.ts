...
export function formatQCRecombinants<TFunction extends TFunctionInterface>(
  ...
  if (clusterGaps && clusterGaps.numClusters >= 2) {
    parts.push(
      t('{{numClusters}} SNP clusters with max gap of {{maxGap}} bp', {
        numClusters: clusterGaps.numClusters,
        maxGap: clusterGaps.maxGap,
      }),
    )
  }
