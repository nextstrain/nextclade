...
namespace klibpp {
  ...
  public:
    ...
    ~KStream() noexcept {
      ...
      delete[] this->m_buf;
      delete[] this->w_buf;
      if (this->close != nullptr)
        this->close(this->f);
    }
    /* Accessors */
    inline unsigned long int counts() const {
      return this->counter;
    }

    inline format::Format get_format() const {
      return this->fmt;
    }
    /* Mutators */
    inline void set_wraplen(unsigned int len) {
      this->wraplen = len;
    }

    inline void set_format(format::Format fmt_) {
      this->fmt = fmt_;
    }
    /* Methods */
    ...
    inline void async_write(bool term = false) noexcept {
...

      {
        ...
        if (!this->fail()) {
          ...
          this->produced = true;
          if (term)
            this->terminate = true; /**< XXX: only set here! */
        }
        this->m_begin = 0;
      }
      this->cv->notify_one();
      ...
      do {
        {
          ...
          this->produced = false;
          if (this->terminate || this->w_end < 0)
            term = true;
        }
        this->cv->notify_one();
      } while (!term);
  ...
  public:
    ...
    inline KStream& operator>>(KSeq& rec)// kseq_read
    {
      ...
      if (!this->getuntil(KStream::SEP_SPACE, rec.name, &c))
        return *this;
  ...
  inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, TSpec> make_kstream(TFile&& file, TFunc&& func, TSpec,
    Args&&... args) {
    return KStream<std::decay_t<TFile>, std::decay_t<TFunc>, TSpec>(std::forward<TFile>(file),
      std::forward<TFunc>(func), TSpec(), std::forward<Args>(args)...);
  }
  ...
  inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::In_> make_ikstream(TFile&& file, TFunc&& func,
    Args&&... args) {
    return KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::In_>(std::forward<TFile>(file),
      std::forward<TFunc>(func), mode::in, std::forward<Args>(args)...);
  }

  template<typename TFile, typename TFunc, typename... Args>
  inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::Out_> make_okstream(TFile&& file, TFunc&& func,
    Args&&... args) {
    return KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::Out_>(std::forward<TFile>(file),
      std::forward<TFunc>(func), mode::out, std::forward<Args>(args)...);
  }
}// namespace klibpp
