...
When available, we will include [WHO labels for VoCs and VoIs](https://www.who.int/en/activities/tracking-SARS-CoV-2-variants/).
...
In addition, we flag sequences in which some of the major genes fail to translate because of frame shifting insertions or deletions.
...
For Nextclade, we therefore compiled the C++ code of nextalign into a web-assembly module that runs in your browser.
...
The alignment is codon aware in the sense that it preferentially opens gaps at the beginning of a codon is this is possible.
...
For most sequences, however, this alignment strategy works well and takes much less than a second per sequence.

In addition to alignments of the nucleotide sequences, nextclade extracts sequences of genes (open reading frames) and separately aligns their translations to translations of the reference sequence.
This allows an unambiguous assignment of amino acid mutations.

The alignments can be downloaded for further analysis.

### Clade Assignment
Clades are groups of related sequences that share a common ancestor.
Nextclade assigns clades by placing your sequence in a tree representing currently circulating SARS-CoV-2.
The clade is then inferred from the point in the tree the sequences attaches to.
On the reference tree, clades are assigned using lists of [clade-defining mutations](https://github.com/nextstrain/ncov/blob/master/defaults/clades.tsv) via the [augur](https://github.com/nextstrain/augur) workflow.


### Credits

 - Artic ncov-2019 Primers are CC-BY licensed. Source: [GitHub](https://github.com/artic-network/artic-ncov2019/blob/b3f2dda5e6d95bc1c5d95a04d4ef37d304479477/primer_schemes/nCoV-2019/V4/SARS-CoV-2.primer.bed#L1).
