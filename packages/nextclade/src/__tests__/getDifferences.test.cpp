...
#include "../../src/tree/treeAttachNodes.h"
...
namespace {
  ...
  using Nextclade::getDifferences;
  using Nextclade::parseMutation;


  class getDifferences : public ::testing::Test {
  protected:
    ...
    std::vector<NucleotideSubstitution> toStrings(
      const std::map<std::string, std::vector<std::string>>& substitutions) {

      std::vector<NucleotideSubstitution> muts;
      for (const auto& sub : substitutions.at("nuc")) {
        const auto mut = parseMutation(sub);
        muts.emplace_back(mut);
      }
      return muts;

      //      return substitutions.at("nuc");
      //      std::vector<std::string> muts;
      //      for (const auto& sub : substitutions.at("nuc")) {
      //        const auto mut = Nextclade::formatMutation(sub);
      //        muts.emplace_back(mut);
      //      }
      //      return muts;
    }
...
TEST_F(getDifferences, Returns_Empty_If_No_Mutations) {
  ...
  const auto actual = toStrings(Nextclade::getDifferences(seq, node, rootSeq, 0.0).mutations);
  const auto expected = makeMutList({});
  EXPECT_ARR_EQ_UNORDERED(actual, expected);
}

TEST_F(getDifferences, Returns_Empty_If_Matching_Single_Element) {
  auto tree = Tree(R"({ "tree": {} })");
...
TEST_F(getDifferences, Returns_Empty_If_Same_muts) {
  auto tree = Tree(R"({ "tree": {} })");
...
TEST_F(getDifferences, Returns_Query_Muts_For_Disjoint_Sets) {
  auto tree = Tree(R"({ "tree": {} })");
...
TEST_F(getDifferences, Returns_Set_Difference_In_General_Case) {
  auto tree = Tree(R"({ "tree": {} })");
