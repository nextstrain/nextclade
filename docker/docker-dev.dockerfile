...
COPY "tools/glibc/out/glibc-2.17.tar.xz" "/opt"

RUN set -euxo pipefail >/dev/null \
&& if [ "$(lsb_release -cs)" == "jessie" ]; then \
  cd /usr \
  && xz -cdk "/opt/glibc-2.17.tar.xz" | tar -xf - \
  && ln -sf /usr/lib/libc.so.6 /lib/x86_64-linux-gnu/libc.so.6 \
  && ln -sf /usr/lib/libc.so.6 /lib/x86_64-linux-gnu/libc-2.19.so \
  && ln -sf /usr/lib/libc.so.6 /lib/x86_64-linux-gnu/libc-2.17.so \
  && ln -sf /usr/lib/libc.so.6 /lib/x86_64-linux-gnu/libc-2.13.so \
;fi
