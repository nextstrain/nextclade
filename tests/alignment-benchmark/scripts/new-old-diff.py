...
def main(
    ...
    base_df = pl.read_csv(
        old_tsv_path, separator="\t", infer_schema_length=10000
    ).sort("index")
    eval_df = pl.read_csv(
        new_tsv_path, separator="\t", infer_schema_length=10000
    ).sort("index")
    ...
    df = df.with_columns(
            alignmentScoreDiff=pl.col("alignmentScore_eval")  - pl.col("alignmentScore") # + pl.col("totalMissing_eval")
    )
    ...
    median = summary[6,1]
    max = summary[8,1]
    better_share = df.filter(pl.col("alignmentScoreDiff") > 0).shape[0] / count
    worse_share = df.filter(pl.col("alignmentScoreDiff") < 0).shape[0] / count
    same_share = df.filter(pl.col("alignmentScoreDiff") == 0).shape[0] / count
    ...
    rundiff = {
        **params,
        ...
        "wall_diff": (new_runtime_df["mean"].sum() - old_runtime_df["mean"].sum()) / count,
        "wall_diff_ratio": new_runtime_df["mean"].sum() / old_runtime_df["mean"].sum(),
        "cpu_share_old": (old_runtime_df["user"].sum() + old_runtime_df["system"].sum()) / old_runtime_df["mean"].sum(),
        "cpu_share_new": (new_runtime_df["user"].sum() + new_runtime_df["system"].sum()) / new_runtime_df["mean"].sum(),
    }
    with open("/dev/stdout", "w") as outfile:
        import csv
        writer = csv.DictWriter(outfile, fieldnames=rundiff.keys(), delimiter="\t")
        writer.writeheader()
